    ...
    for col_name in ["project_archive", "decisions", "failures", "sessions", "entities"]:
        try:
            ...
            for hit in hits:
                ...
                if chromadb_client.upsert_document(col_name, doc_id, new_content, metadata):
                    records_affected += 1
        except Exception as e:
...
async def context_correct(request: CorrectRequest):
    ...
    return CorrectResponse(item=request.item, correction=request.correction, hot_updated=hot_updated, archive_updated=records_affected > 0, records_affected=records_affected, message=f"Correction: {'; '.join(parts)}.")
