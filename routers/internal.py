...
async def health():
    ...
    return HealthResponse(status="healthy" if dm.level.value in ("full", "partial") else "degraded",
                          version="0.3.0", chromadb_connected=chromadb_client.is_connected(),
                          kb_accessible=kb_gateway.kb_accessible(), sessions_count=sessions_count,
                          uptime_seconds=round(time.time() - _start_time, 1), learning_mode=LEARNING_MODE,
...
async def get_summary():
    dm = get_degradation_manager()
    content = kb_gateway.read_master_context() or dm.get_cached_context()
    if not content:
        return {"summary": "Master context not yet created.", "tokens_estimate": 10, "degraded": True}
    summary = content[:2000] + "\n[truncated]" if len(content) > 2000 else content
    return {"summary": summary, "tokens_estimate": len(summary.split()), "degraded": dm.level.value != "full"}
...
async def get_stats():
    ...
    try:
        for f in sorted(SESSIONS_DIR.glob("*.json"), key=lambda f: f.stat().st_mtime, reverse=True):
            ...
            try:
                ...
                if sd.get("_processed"): processed_count += 1
                else: unprocessed_count += 1
                if len(recent_sessions) < 20:
                    recent_sessions.append({"session_id": sd.get("session_id", f.stem), "significance": sd.get("significance"),
                                            "saved_at": sd.get("saved_at"), "processed": bool(sd.get("_processed")),
                                            "summary_preview": (sd.get("summary") or "")[:120]})
            except Exception: unprocessed_count += 1
    ...
    return {"sessions": {"total": sessions_count, "processed": processed_count, "unprocessed": unprocessed_count},
            "sessions_total": sessions_count, "recent_sessions": recent_sessions,
            "chromadb_collections": chromadb_stats, "kb_accessible": kb_gateway.kb_accessible(),
            "learning_mode": LEARNING_MODE, "worker": get_processor().status, "llm": llm_stats}


@router.get("/api/worker")
async def worker_status():
    result = get_processor().status
    try:
        from services.openrouter import get_client as get_llm
        result["llm"] = get_llm().stats
    except Exception: result["llm"] = {"error": "not initialized"}
    return result
...
async def dismiss_nudge_endpoint(request: dict):
    ...
    return {"dismissed": dismiss_nudge(msg), "query": msg} if msg else {"error": "message required"}
...
async def dismiss_anomaly_endpoint(request: dict):
    ...
    return {"dismissed": dismiss_anomaly(desc), "query": desc} if desc else {"error": "description required"}
...
async def claude_desktop_config():
    return {"config": {"mcpServers": {"context-engine": {"command": "python3", "args": ["<path>/mcp-bridge.py"],
            "env": {"CONTEXT_ENGINE_URL": "http://localhost:9040"}}}}}
