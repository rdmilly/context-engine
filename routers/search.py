...
async def context_search(request: SearchRequest):
        ...
        try:
            ...
            for hit in hits:
                if hit.get("distance", 999) < 1.8:
                    result = {"collection": col_name, "id": hit["id"], "content": hit["content"], "metadata": hit.get("metadata", {}), "distance": hit.get("distance"), "relevance": round(1.0 - (hit.get("distance", 1.0) / 2.0), 3)}
    ...
    return SearchResponse(query=request.query, results=all_results, total_results=len(all_results), collections_searched=target_collections)
